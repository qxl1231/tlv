...
var statusSpec = function (reportType) {
    if (reportType == '02CA') {
        return {
            ca_appointment_wash: {type: 0xB6, len: 1, valueCalcFunc: oneValue},//预约开关
            ca_pay_result: {type: 0xA0, len: 1, valueCalcFunc: oneValue},//付费结果

            ca_balance: {type: 0xB0, len: 2, valueCalcFunc: twoValue},//余额
            ca_fee_price: {type: 0xB1, len: 2, valueCalcFunc: twoValue},//扣费金额

            ca_pwd: {type: 0xB4, len: 7, valueCalcFunc: pwdValue},//密码};
            ca_mode_price: {type: 0xB5, len: 0x03, valueCalcFunc: modeValue}
        }
    }
...
var commandSpec = {
    ca_appointment_wash: {type: 0xB6, len: 0x01, valueCalcFunc: valueOne},//预约开关
    ca_pay_result: {type: 0xA0, len: 0x01, valueCalcFunc: valueOne},//付费结果

    ca_balance: {type: 0xB0, len: 0x02, valueCalcFunc: valueTwo},//余额
    ca_fee_price: {type: 0xB1, len: 0x02, valueCalcFunc: valueTwo},//扣费金额

    ca_pwd: {type: 0xB4, len: 0x07, valueCalcFunc: valuePwd},//密码
    ca_mode_price: {type: 0xB5, len: 0x03, valueCalcFunc: valueMode}
